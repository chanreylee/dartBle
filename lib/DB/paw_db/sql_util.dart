...
class SqlUtil extends BaseModel {
  ...
  Future<int> rawInsert(String sql, List arguments) async {
    int id = await this.db.rawInsert(sql, arguments);
    // pushNotification();
    return id;
  }
  ...
  Future<int> insert(Map<String, dynamic> params) async {
    int id = await this.db.insert(tableName, params);
  ...
  void pushNotification(int db_type, dynamic arguments) {
    // 'library','directory','mediaInfo','mediaDetailedInfo','device'
    
    // eventBus.fire(PawDBEvent());
  }
}
